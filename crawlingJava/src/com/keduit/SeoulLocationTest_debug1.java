...
public class SeoulLocationTest_debug1 {
	List<SeoulLocationVO> list = new ArrayList<>();
	private String nullValue = "없음";

	public static void main(String[] args) {
		SeoulLocationTest_debug1 st = new SeoulLocationTest_debug1();
		st.seoulLocation();
	...
	private void seoulLocation() {
		...
		try {
			Element pages = Jsoup.connect("https://korean.visitseoul.net/attractions").get().select("div.paging-lst a")
					.last();
			...
			for (int i = 1; i <= pageCount; i++) {
				...
				for (int j = 0; j < thumb.size(); j++) {
					...
					Document docSub = Jsoup
							.connect("https://korean.visitseoul.net" + link.get(j).attr("href").toString()).get();
					// 첫번째 목록
					...
					// 없다면 -1이 저장되므로 값 체크
					String phone = phoneNum == -1 ? nullValue
							: docSub.select("div.detail-map-infor:eq(0) dl dd").get(phoneNum).text();
					String info = infoNum == -1 ? nullValue
							: docSub.select("div.detail-map-infor:eq(0) dl dd").get(infoNum).text();
					...
					// 없다면 -1이 저장되므로 값 체크
					String address = addrNum == -1 ? nullValue
							: docSub.select("div.detail-map-infor:eq(1) dl dd").get(addrNum).text();
					String traffic = trafficNum == -1 ? nullValue
							: docSub.select("div.detail-map-infor:eq(1) dl dd").get(trafficNum).text();
...
// 상용님 코드
// 장점 : 정보가 없으면 없음 처리 된다, 전화번호/이용시간 체크로 코드가 간결해짐